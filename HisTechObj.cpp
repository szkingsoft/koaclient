...
//////////////////////////////////////////////////////////////////////////
CHisTechObj * g_pCurrentObj = NULL;
//////////////////////////////////////////////////////////////////////////
int CALLBACK  GetYFromValue(double dValue)
{
	if(NULL == g_pCurrentObj)return 0;
	return g_pCurrentObj->GetPic(0)->GetStationFromPrice(dValue);
}
int CALLBACK  GetXFromTime(CTime time) 
{
	if(NULL == g_pCurrentObj)return 0;
#ifndef _DEBUG
	return g_pCurrentObj->GetPic(0)->GetStationFromDate(time.GetTime());
#else
	return g_pCurrentObj->GetPic(0)->GetStationFromDate(time.GetTime());
#endif

}
double CALLBACK GetValueFromY(int y)        
{
	if(NULL == g_pCurrentObj)return 0;
	return g_pCurrentObj->GetPic(0)->GetPriceFromStation(y);
}
CTime CALLBACK GetTimeFromX(int x)          
{
	if(NULL == g_pCurrentObj)return 0;
#ifndef _DEBUG
	return g_pCurrentObj->GetPic(0)->GetDateFromStation(x);
#else
	return g_pCurrentObj->GetPic(0)->GetDateFromStation(x);
#endif
}
void CALLBACK   GetKLineRect(CRect &rect)      
{
	if(NULL == g_pCurrentObj)return ;
	g_pCurrentObj->GetPic(0)->GetDrawRect(rect);
}
double CALLBACK GetUnitLength()           
{
	if(NULL == g_pCurrentObj)return 0.0f;

	return g_pCurrentObj->GetPic(0)->GetUnitLength();
}

BOOL CALLBACK  GetHighLowFromTime(const CTime & time,double &nHigh,double &nLow)
{
	if(NULL == g_pCurrentObj)return FALSE;

	return g_pCurrentObj->GetPic(0)->GetHighLowFromTime(time.GetTime(),(float &)nHigh,(float &)nLow);
}

void CALLBACK   GetCloseFromXSgmt( const int &uBeginX,const int &uEndX, CArray< CPoint,CPoint&> &arPtClose )//获得指定坐标点内的所有收盘价
{
	if(NULL == g_pCurrentObj)return ;
	int nBeginTime,nEndTime;
	nBeginTime = g_pCurrentObj->GetPic(0)->GetDateFromStation(uBeginX);
	nEndTime = g_pCurrentObj->GetPic(0)->GetDateFromStation(uEndX);
	g_pCurrentObj->GetPic(0)->GetCloseFromXSgmt( nBeginTime, nEndTime, arPtClose );
}
BOOL CALLBACK   GetHighLowFromXSgmt( const UINT uBeginX, const UINT uEndX, CPoint &ptHigh, CPoint &ptLow )//获得X方向范围内的最高最低价对应的坐标点
{
	if(NULL == g_pCurrentObj)return FALSE;
	int nBeginTime,nEndTime;
	nBeginTime = g_pCurrentObj->GetPic(0)->GetDateFromStation(uBeginX);
	nEndTime = g_pCurrentObj->GetPic(0)->GetDateFromStation(uEndX);
	return g_pCurrentObj->GetPic(0)->GetHighLowFromXSgmt(nBeginTime,nEndTime, ptHigh, ptLow );
}
...
//	m_bTranslateIsPoint = TRUE;
	...
	m_bMousePress = FALSE;
	m_ptCurPress = CPoint(0,0);
	g_pCurrentObj= this;
	////////////////////////////////
	SetDrawLineCallBcakFun();
}
void CHisTechObj::SetCurSymbol(CString strSymbolCode)
{
	USES_CONVERSION;
	CPublic::Instance()->GetDrawDll()->m_pSetCurSymbol(T2W(strSymbolCode));
}
void CHisTechObj::SetDrawLineCallBcakFun()
{
	CPublic::Instance()->GetDrawDll()->m_pSetCallbakGetYFromValue(&(GetYFromValue));
	CPublic::Instance()->GetDrawDll()->m_pSetCallbakGetXFromTime(&GetXFromTime);
	CPublic::Instance()->GetDrawDll()->m_pSetCallbakGetValueFromY(&GetValueFromY);
	CPublic::Instance()->GetDrawDll()->m_pSetCallbakGetTimeFromX(&GetTimeFromX);
	CPublic::Instance()->GetDrawDll()->m_pSetCallbakGetUnitLength(&GetUnitLength);
	CPublic::Instance()->GetDrawDll()->m_pSetCallbakGetKLineRect(&GetKLineRect);
	CPublic::Instance()->GetDrawDll()->m_pSetCallbakGetHighLowFromTime(&GetHighLowFromTime);
	CPublic::Instance()->GetDrawDll()->m_pSetCallbakGetCloseFromXSgmt(&GetCloseFromXSgmt);
	CPublic::Instance()->GetDrawDll()->m_pSetCallbakGetHighLowFromXSgmt(&GetHighLowFromXSgmt);
}
CHisTechObj::~CHisTechObj()
{
	DestoryData();
...
//函数参数：无
//函数返回：void
void CHisTechObj::RecalSubObjSize(int iRightW )
{ 
...
//	KLineRect.left += VERTBANKSIZE_HISOBJ;//保留左侧文字区域	
	KLineRect.right -= iRightW/*VERTBANKSIZE_HISOBJ*/;//保留左侧文字区域
	/////////////////////////////////////////////////
...
// 	m_TipInfoRect.left = m_DrawRect.right - VERTBANKSIZE_HISOBJ;
	...
	m_TipInfoRect.bottom = m_DrawRect.top+18*7+20/*16*16+4+16*/;
...
void CHisTechObj::SetStockInfo(const SCodeInfo* pCodeInfo)
{
	...
	if(m_nCanShowMaxKUnits > 200)
		m_nCanShowMaxKUnits = 200;

	m_sLastSize = /*100*/m_nCanShowMaxKUnits+ m_nMaxParameret;
	m_nLeftMoveRequestStatus = NORMALREQUEST ;
...
//	m_bRequestedData = TRUE;
	...
	//TRACE("向服务器请求历史数据 Index = %d  %02d:%02d:%02d\r\n", pReq->m_usIndex, time.GetHour(), time.GetMinute(), time.GetSecond()); 
	CPublic::Instance()->SetRequestHisDataFlag(true);
	CPublic::Instance()->SendRequestData(pBuffer,nBufferSize); 
	//////////////////////////////////////////////////////////////////////////			
	delete []pBuffer; 
}
void CHisTechObj::RequestData_ZYQ()
{
	if((NULL ==  m_pCodeInfo))	return; 
	int nBufferSize;
	nBufferSize = sizeof(SNormaHead)+sizeof(SHisotyDataReq)+sizeof(SCodeInfo);
	...
	ZeroMemory(pBuffer,nBufferSize);
	SNormaHead* pReq  = (SNormaHead*)pBuffer;
	pReq->m_usType = HISKDATA_ZYQ;//后续请求
	pReq->m_usIndex = CPublic::Instance()->AddRequestIndex();
	pReq->m_usSize = 1;
	SHisotyDataReq* pHistoyReq;
	pHistoyReq = (SHisotyDataReq*)(pReq+1);
...
//			m_bRequestedData = FALSE;
	...
	case HISKDATA_ZYQ:
		{
			TreatDayDataArrived_ZYQ((char*)pData);
		}
		break;
...
{ 
	if(m_nDataEnd > m_nDataBegin)
	{
		...
		m_fCurrentWidth /=(m_nDataEnd - m_nDataBegin+1);//除去左右侧保留宽度,K线之间的有效宽度
	}
	else
	{
		m_fCurrentWidth = 10;
	}
	m_nRecsOfUnit = (m_nDataEnd-m_nDataBegin+1)>m_TimeRect.Width()?(m_nDataEnd-m_nDataBegin+1)/m_TimeRect.Width()+1:1;
	//下面得到K线有效间隙
	...
	m_nCanShowMaxKUnits = m_TimeRect.Width()/m_fCurrentWidth;
}
...
void CHisTechObj::ZoomOutData()
{
	...
	int nDisplayNum = m_nCanShowMaxKUnits;
	int iDsNum = 11;
	if(nDisplayNum <iDsNum)return;

	nDisplayNum =nDisplayNum*10/iDsNum;
	//////////////////////////////////////////////////////////////////////////
	//修改部分  11-03
...
{
	if(m_pRecvKUnit == NULL || CPublic::Instance()->GetRequestHisDataFlag())return;//请求后没有返回数据
	if(m_nRecsOfUnit>2)return;//单根K线大于2根拒绝请求
	if(TP_END == m_sRequestStatus && m_nDataBegin <= 0 && m_nDataEnd >= (m_nValueNum-1)) return;
	...
	//////////////////////////////////////////////////////////////////////////
	//当前位置
	int nTempDataBegin = m_nDataBegin;
	int nDisplayDataNum;
	nDisplayDataNum = m_nDataEnd -m_nDataBegin;
	nDisplayDataNum =nDisplayDataNum*11/10;	
	m_nDataBegin = m_nDataEnd - nDisplayDataNum;

	if(m_nDataBegin <= m_nMaxParameret)
	{
		if(TP_END == m_sRequestStatus)
		{
			m_nDataBegin = 0;
			m_nDataEnd = m_nDataBegin+nDisplayDataNum;
			if(m_nDataEnd > (m_nValueNum-1))
				m_nDataEnd = m_nValueNum-1;
		}
		else
		{
			m_sLastSize = 200-m_nDataBegin;
			m_nLeftMoveRequestStatus = ZOOMINREQUEST;
		//	m_nDataBegin = 0;
			m_nDataBegin = nTempDataBegin;
			RequestData();
			return;
		}
	}
...
void CHisTechObj::TransCurStationToPic()
{
	...
	if(m_nVertStation != -1)
	{
		if(m_nValueNum <= m_nDataBegin +m_nVertStation)
			return;
		...
		//传入了表示数据更新 是来自历史技术分析 而不是分时数据 modify by witming 
		//m_InfoTable.UpdateData(&seq,0);
		 m_InfoTable.UpdateDataFromHis(&seq,0);
	}
}
...
void CHisTechObj::TransKDataToPic()
{
	...
	seq.m_nSize = m_nDataEnd+1;//m_nSize-m_nDataBegin能够得到需要显示的数据量
	...
	//////////////////////////////////////////////////////////////////////////
 	m_nHoriPoint = m_nDataEnd - m_nDataBegin + 1;//数据量
...
BOOL CHisTechObj::OnUserCmd(UINT iSel)
{
	...
	case ID_FUNCTION_WR:
	case ID_FUNCTION_MA:
	case ID_FUNCTION_VOL:
		{
			char szInfo[10][6]={"MACD","RSI","BOLL","KDJ","DMI","WR","MA","VOL"};			
	...
	case ID_TECHANALY_MONLINE: //月线
		{
			...
			m_nDataBegin = m_nDataEnd = 0;
			SetStockInfo(NULL);
			m_InfoTable.Init(m_nCircleInfo);
			RequestData();
		}
		break;
	...
	case ID_INDEX_MODIFYFUNCTION: //调整指标公式
		{
			...
			if (CFormulaFace::OnOpenDefineTechDlg(paras,(char*)(LPCTSTR)strTech,(CWnd*)pWnd) == IDOK)
			{
				m_pHisCurvePic[m_nCurrentTechIndex].UpdateTechNameTitle();
				RecalSubObjSize();
				TransCurStationToPic();
...
//				m_nDrawState = ENDDRAW;
	...
	default:
		{
			if((iSel >=ID_DRAWLINE_BEGINTAG_Z)&&(iSel <=ID_DRAWLINE_ENDTAG_Z))
			{
				TRACE("isel =%d",iSel);
				if(iSel == ID_DRAWLINE_REDO_Z)
				{
					(*CPublic::Instance()->GetDrawDll()->m_pNextStep)();
				}
				else if(iSel == ID_DRAWLINE_UNDO_Z)
				{
					(*CPublic::Instance()->GetDrawDll()->m_pPreStep)();
				}
				else
				{
					(*CPublic::Instance()->GetDrawDll()->m_pCreateGraph)(iSel - ID_DRAWLINE_BEGINTAG_Z);
				}
			}
		}
		return FALSE;
...
BOOL CHisTechObj::OnUpdateUserCmd(CCmdUI *pCmdUI)
{
	...
	default:
		...
		{
			if((pCmdUI->m_nID == ID_DRAWLINE_UNDO_Z))
			{
				pCmdUI->Enable/*SetCheck*/((*CPublic::Instance()->GetDrawDll()->m_pCanPreStep)());
			}
			else if(pCmdUI->m_nID == ID_DRAWLINE_REDO_Z)
			{
				pCmdUI->Enable((*CPublic::Instance()->GetDrawDll()->m_pCanNextStep)());
			}
			else
			{
				pCmdUI->Enable(TRUE);
...
{
	m_nCircleInfo = DAY_HISK;//设置K线为日线
	m_InfoTable.Init(m_nCircleInfo);//设置K线数据状态
    m_nCurKLinePriodID = ID_TECHANALY_DAYLINE;
}
...
{	
	...
	case VK_LEFT:
		{//左移光标	
			...
			//CPublic::Instance()->GetView(/*VIEWINFO_STOCK*/10)->ClientToScreen(&station);
	...
	(*CPublic::Instance()->GetDrawDll()->m_pKeyDown)(nChar);
	return TRUE;
}
...
BOOL CHisTechObj::MouseLBtnDown(CPoint point)
{
	...
	if (m_TimeRect.PtInRect(point))
	{
		m_ptCurPress = point;
		m_bMousePress = TRUE;
		CRect rcWnd = m_TimeRect;
	...
	if((*CPublic::Instance()->GetDrawDll()->m_pLButtonDown)(point))return TRUE;
	//////////////////////////////////////////////////////////////////////////
...
BOOL CHisTechObj::MouseLBtnUp(CPoint point)
{
	...
	if((*CPublic::Instance()->GetDrawDll()->m_pLButtonUp)(point))return TRUE;

	int nCount;
	for(nCount= 0;nCount<m_nCurrentPicNum;nCount++)
	{
		if (m_pHisCurvePic[nCount].MouseUp(point))
		{
			return TRUE;
		}
	}
...
BOOL CHisTechObj::MouseMove(CPoint point)
{
	if (m_TimeRect.PtInRect(point))
	{
		...
		{
			CWnd *pWnd = CPublic::Instance()->GetView(10);
			if (!pWnd||!IsWindowVisible(pWnd->GetSafeHwnd()))break;
			HCURSOR hCur  =  LoadCursor( NULL  , IDC_SIZEWE ) ;
			::SetCursor(hCur);
			int iEveryWith = m_fCurrentWidth+m_nKUnitWidth/2;
			if (!m_bMousePress)break;
			if ((point.x - m_ptCurPress.x) >= iEveryWith)
			{
				m_bDrawCross = TRUE;
				m_ptCurPress = point;
				m_nVertStation = 0;
			...
			else if ((point.x - m_ptCurPress.x) <= -iEveryWith)
			{
				m_bDrawCross = TRUE;
				m_ptCurPress = point;
				m_nVertStation = m_nHoriPoint;
				RightMove();
			}
		} while (0);
		return TRUE;
	...
	if((*CPublic::Instance()->GetDrawDll()->m_pMouseMove)(point))return TRUE;
...
BOOL CHisTechObj::MouseRBtnDown(LPVOID lpWnd,CPoint point)
{
	...
	if ((*CPublic::Instance()->GetDrawDll()->m_pSetGraphicProperty)(pViewWnd,point))return TRUE;
	//////////////////////////////////////////////////////////////////////////
	//判断点击点
	BOOL bClickIndex;
	bClickIndex = FALSE;
...
void CHisTechObj::DrawCrossCursor(CDC *pDC)
{
	...
	{//有效区域内
		...
		if( GetCurCrossCursorPosValue(szText,fValue))
		{
			...
			CPen newPen1(PS_SOLID,1,RGB(175,50,50/*200,200,200*/));
			CPen* OldPen1 = pDC->SelectObject(&newPen1);
			CRect rcFill(ItemRect);
			rcFill.InflateRect(3,3,3,3);
			pDC->Rectangle(rcFill);
			rcFill.InflateRect(-1,-1,-1,-1);
			pDC->FillSolidRect(rcFill,RGB(0,0,255));
		...
		szText = CDateManage::Instance()->FormatString(nDate,m_nCircleInfo);
		ItemRect = m_TimeRect;
...
// 	if(IsBadReadPtr(m_pRecvKUnit,m_nHoriPoint*m_nRecsOfUnit*sizeof(SHistoryKUnit))return;
	...
	CRect	ItemRect;//单元区域
	COLORREF crText;
	crText  = RGB(255,50,50);//CInterfaceManager::Instance()->GetSysColor(ID_COORDTEXT_COLOR);
	CFont* TextFont = CInterfaceManager::Instance()->GetSysFont(ID_NUMBER_FONT);
	CFont* OldFont = pDC->SelectObject(TextFont);
	pDC->SetTextColor(crText);
	int nOldMode = pDC->SetBkMode(TRANSPARENT);
	COLORREF crLine;
	crLine = RGB(176,0,0);//CInterfaceManager::Instance()->GetSysColor(ID_COORDTEXT_COLOR);
	...
	for(nCount = 0;nCount<m_nHoriPoint;nCount++)
	{
		...
		if(ItemRect.left <TextRect.right+30 )
		{
			iGapNum++;
			continue;
		}
		if (iLeft > 0 && TextRect.right > 0)
		{
			int iEvWith = (TextRect.right + 30 - iLeft)/4;
			for (int i = 1; i < 4; i++)
			{
				pDC->MoveTo(iLeft+i*iEvWith,rRect.top);
				pDC->LineTo(iLeft+i*iEvWith,rRect.top+iShortScale);
			}
		}
		iLeft = ItemRect.left;
		iGapNum = 0;
		...
		{//画出分钟线
			...
			{//第一次画出月日信息
				...
				pDC->LineTo(ItemRect.left,rRect.top+iLongScale);
				//////////////////////////////////////////////////////////////////////////				
				//szItem.Format(_T("%d"),nDate1);//日期信息
				/////
				CTime tmFormat(2014,10,9,nDate1%10000/100,nDate1%10000%100,0);
				//szItem.Format(_T("%d:%d"),nDate1%10000/100,nDate1%10000%100);//日期信息
				szItem = tmFormat.Format(_T("%H:%M"));
				//////
...
//	if(m_bDrawCross&&m_bMouseInRect)
	...
	(*CPublic::Instance()->GetDrawDll()->m_pDraw)(pDC);
	return TRUE;
}
...
void CHisTechObj::LeftMove()
{
	...
	if((m_nVertStation == 0))
	{
		if(m_nDataBegin <= m_nMaxParameret && m_sRequestStatus != TP_END)
		{//请求数据
			m_sLastSize = 200;
		...
		if(m_nDataEnd - m_nDataBegin + 1 > m_nCanShowMaxKUnits)  // 当放大到最大时要先填满空白的K线
			m_nDataEnd --;
	...
	if( m_pHisCurvePic[0].GetPriceFromStation(m_nHoriStation) == -1)
	{
		...
		m_nHoriStation = m_pHisCurvePic[0].GetStationFromPrice(nValue);//得到当前位置
	}
...
//	if(m_bRequestedData||(m_pRecvKUnit == NULL))return;//请求后没有返回数据
	...
	int nDisplayNum;
	nDisplayNum = m_nDataEnd-m_nDataBegin+1;
	if(m_nDataBegin < (nDisplayNum+m_nMaxParameret) && m_sRequestStatus != TP_END)
	{
	...
	/////////////////////////////////////////////////////////////////////////
	if(nDisplayNum < m_nCanShowMaxKUnits)  // 当放大到最大时要先填满空白的K线
		nDisplayNum = m_nCanShowMaxKUnits;
...
{
	if((m_pRecvKUnit == NULL))return;//请求后没有返回数据
	if(m_nDataEnd >= m_nValueNum - 1) return;//数据到底直接返回	

	m_nDataEnd = m_nDataEnd + m_nCanShowMaxKUnits;	

	if(m_nDataEnd >= m_nValueNum)
		m_nDataEnd = m_nValueNum - 1;

	m_nDataBegin = m_nDataEnd - m_nCanShowMaxKUnits + 1;
...
BOOL CHisTechObj::MakeDayData(StuRecvQuoteInfo* pQuote)
{
	...
	m_pRecvKUnit[m_nValueNum-1].m_uiVolume = 1000 + rand()%100;
	return TRUE;
}
//BOOL CHisTechObj::MakeDayData(StuRecvQuoteInfo* pQuote)
//{
...
//		timevalue.SetCurrentDate(CPublic::Instance()->GetDate());
//		int nHistory;
//		nHistory = quotetime.GetHour()*100+quotetime.GetMinute();
//		int nDay;
//		nDay= quotetime.GetMonth()*100+quotetime.GetDay();
//		int nNewTime;
//		nNewTime = nDay*10000+nHistory;
//		// add by david gong at 20140126
//		int nLastTime;
//		nLastTime = m_pRecvKUnit[m_nValueNum-1].m_uiDate;
//		BOOL bCondication = FALSE;
//		if((nNewTime<(nLastTime+m_nCircleInfo))&&(nNewTime%10000 != 0))bCondication = TRUE;
//		if((nNewTime == 0)&&(nLastTime ==nNewTime)) bCondication= TRUE;
//    	if(!bCondication)
//		//////////////////////////////////////////////////////////////////////////
//		{//超过当前周期
//			SHistoryUnit * pTemp = new SHistoryUnit[m_nValueNum+1];
...
//			{
//				int nTransTime;
//				nTransTime = nHistory/100*60+nHistory%100;
//				nTransTime = (nTransTime/m_nCircleInfo)*m_nCircleInfo;
//				nTransTime = nTransTime/60*100+nTransTime%60;
//				nCurrentDate =nDay*10000+ nTransTime;	
//			}
//			else
//			{
//				nCurrentDate =nDay*10000+ nHistory;		
//			}
//
//			m_pRecvKUnit[m_nValueNum-1].m_uiDate = nCurrentDate;
//			m_pRecvKUnit[m_nValueNum-1].m_fMaxPrice = fNewPrice;
...
BOOL CHisTechObj::TreatDayDataArrived_ZYQ(const char* pHisotyData)
{
	return FALSE;
	SNormaHead * pDayHead = (SNormaHead*)(pHisotyData);
	STUSTKMinUnitZYQ* pDayData = (STUSTKMinUnitZYQ*)(pDayHead+1);
	...
	for(int nCount = 0;nCount< pDayHead->m_usSize;nCount++)
	{
		...
		//pTemp[nCount].m_uiVolume = pDayData[nCount].uiTrunoverQty;//1000 + rand()%100;
	...
	{//左移光标
		...
		m_nDataEnd = m_nDataBegin+(m_nHoriPoint)*1/*m_nRecsOfUnit*/;
		m_nLeftMoveRequestStatus = NORMALREQUEST;
...
BOOL CHisTechObj::TreatDayDataArrived(const char *pHisotyData)
{
	...
	for(int nCount = 0;nCount< pDayHead->m_usSize;nCount++)
	{
		...
		pTemp[nCount].m_uiSum = pDayData[nCount].uiTime;
		pTemp[nCount].m_uiVolume = 1000 + rand()%100;
		pTemp[nCount].m_fClosePrice = (float)pDayData[nCount].uiClosePrice/fPow;
		pTemp[nCount].m_fMaxPrice = (float)pDayData[nCount].uiMaxPrice/fPow;
		pTemp[nCount].m_fMinPrice = (float)pDayData[nCount].uiMinPrice/fPow;
		pTemp[nCount].m_fOpenPrice = (float)pDayData[nCount].uiOpenPrice/fPow;
	...
	{//当得到的数据小于得到的数据
		m_sRequestStatus = TP_END;//数据请求结束
		m_sLastSize = pDayHead->m_usSize;
	}

	m_nDataBegin += m_sLastSize;
	m_nDataEnd += m_sLastSize;
	...
	{//Ctrl左移
		m_nDataBegin -= m_nCanShowMaxKUnits;
		if(m_nDataBegin < 0)
			m_nDataBegin = 0;

		m_nDataEnd += (m_nCanShowMaxKUnits - 1);
...
// 			if(m_nDataBegin<0)m_nDataBegin = 0;
	...
	else if (ZOOMINREQUEST == m_nLeftMoveRequestStatus)
	{
		int nDisplayDataNum;
		nDisplayDataNum = m_nDataEnd -m_nDataBegin;
		nDisplayDataNum =nDisplayDataNum*11/10;	
