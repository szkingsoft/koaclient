...
{
	if(time <m_pKUnit[m_nDataBegin].m_uiDate)return FALSE;//小于开始时间
	for(int j=m_nDataBegin;j<m_nValueNum;j++)
	{
...
{
	arPtClose.RemoveAll();
	if(uEndX <m_pKUnit[m_nDataBegin].m_uiDate)return ;//小于开始时间
	CPoint ptClose;
	for(int j=m_nDataBegin;j<m_nValueNum;j++)
	{
...
{
	if(uEndX <m_pKUnit[m_nDataBegin].m_uiDate)return FALSE;//小于开始时间
	BOOL bFrist= TRUE;
